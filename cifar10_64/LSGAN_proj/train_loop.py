...
class TrainLoop(object):
	...
	def train_step(self, batch):
		...
		if self.train_mode == 'hyper':
			...
			for disc in self.disc_list:
				losses_list.append(F.mse_loss(disc.forward(out).squeeze(), y_real_))
	...
	def define_nadir_point(self):
		...
		for disc in self.disc_list:
			...
			disc_outs.append(F.mse_loss(d_out, y_real_).data[0])
	...
	def compute_steepest_direction_norm(self):
		...
		for disc in self.disc_list:
			self.model.zero_grad()
			out = self.model.forward(z_)
			loss = F.mse_loss(disc.forward(out).squeeze(), y_real_)
