...
from common.discriminators import Discriminator_mnist_noproj
...
transform = transforms.Compose([transforms.Resize((28, 28), interpolation=Image.BICUBIC), transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])
...
disc = Discriminator_mnist_noproj(optim.Adam, args.lr, (args.beta1, args.beta2)).train()
fid_model = cnn().eval()
...
trainer = TrainLoop(generator, fid_model, disc, optimizer, train_loader, checkpoint_path=args.checkpoint_path, checkpoint_epoch=args.checkpoint_epoch, cuda=args.cuda)
