...
parser.add_argument('--data-path', type=str, default='./data/tiny-imagenet-200/train/', metavar='Path', help='Path to data')
...
transform = transforms.Compose([transforms.Resize((64, 64), interpolation=Image.BICUBIC), transforms.RandomHorizontalFlip(), transforms.ToTensor(), transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))])

data_set = datasets.ImageFolder(args.data_path, transform=transform)

train_loader = torch.utils.data.DataLoader(data_set, batch_size=args.batch_size, shuffle=True, num_workers=args.workers)
