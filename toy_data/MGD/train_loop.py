...
class TrainLoop(object):
	...
	def __init__(self, generator, disc_list, optimizer, toy_dataset, centers, cov, train_loader, alpha=0.8, nadir_slack=1.1, train_mode='vanilla', checkpoint_path=None, checkpoint_epoch=None, cuda=True, job_id=None):
		...
		self.history = {'gen_loss': [], 'gen_loss_minibatch': [], 'disc_loss': [], 'disc_loss_minibatch': [], 'FD': [], 'quality_samples': [], 'quality_modes': [], 'steepest_dir_norm': []}
		self.total_iters = 0
		self.cur_epoch = 0
	...
	def train(self, n_epochs=1, save_every=1):
		...
		while (self.cur_epoch < n_epochs):
			...
			fd_, q_samples_, q_modes_, st_dir_norm = self.valid()
	...
	def train_step(self, batch):
		...
		if self.train_mode == 'hyper':
			...
			result = minimize(MGD_utils.steep_direct_cost, curr_alpha, args = grads_list, jac = MGD_utils.steep_direc_cost_deriv, constraints = self.constraints, method = 'SLSQP', options = {'disp': False})
	...
	def valid(self):
		...
		return fd, q_samples, q_modes, steepest_dir_norm
